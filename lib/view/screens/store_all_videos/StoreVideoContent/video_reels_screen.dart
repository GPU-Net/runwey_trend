...
import 'package:runwey_trend/view/screens/store_all_videos/StoreVideoContent/tiktok_video_controller.dart';
...
class StoreVideoReelsScreen extends StatefulWidget {
  const StoreVideoReelsScreen({Key? key,required this.initIndex,required this.userId}) : super(key: key);


  final int initIndex;
  final String userId;
  @override
  State<StoreVideoReelsScreen> createState() => _StoreVideoReelsScreenState();
}

class _StoreVideoReelsScreenState extends State<StoreVideoReelsScreen> {
  ...
  final _videoReelsController=Get.put(StoreAllVideoController());
var _currentPage=0;
  ...
  void initState() {
    super.initState();
    setState(() {
      if(_videoReelsController.allStoreContentList.length-1==widget.initIndex){
        _videoReelsController.loadMore(widget.userId);
      }
            ...
            body: Stack(
              children: [
                PageView.builder(
                  ...
                  itemCount:_videoReelsController.isLoadMoreRunning.value? _videoReelsController.allStoreContentList.length+1:_videoReelsController.allStoreContentList.length,
                  onPageChanged: (int page) {

                    if(_videoReelsController.allStoreContentList.length>page){
                      debugPrint("=========> Page Number : $page");
                      _videoReelsController.handleView(_videoReelsController.allStoreContentList[page].id!);
                      if (page == _videoReelsController.allStoreContentList.length - 1) {
                        _videoReelsController.loadMore(widget.userId);
                  ...
                  itemBuilder: (context, index) {
                    if(_videoReelsController.allStoreContentList.length>index){
                      return OrientationBuilder(
                        builder: (context, orientation) {
                          ...
                          return Stack(
                            children: [
                              SizedBox(
                                  child:StoreTikTokVideoController(contentModel:_videoReelsController.allStoreContentList[index],)
                              ),
                              // Positioned(
                              //   bottom: 20,
                              //   left: 20,
